...
def process_data(f_mkt, f_crm, f_ml):
    ...
    with t1:
        ...
        with c1: st.table(mkt_summary_df)
        with c2: st.plotly_chart(px.pie(mkt_summary_df, values='Số lượng', names='Hạng mục', title="Tỷ lệ Lead MKT"), use_container_width=True)
    ...
    with t2:
        ...
        st.plotly_chart(px.bar(df_c_f['STATUS'].value_counts().reset_index(), x='index', y='STATUS', title="Số lượng theo Status"), use_container_width=True)

    with t3:
        st.subheader("Hiệu suất Doanh thu (Masterlife Data)")
    ...
    # --- TÍNH NĂNG EXPORT 3 SHEETS ---
    st.sidebar.markdown("---")
    if st.sidebar.button("📥 Export 3 Sheets (Excel)"):
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            mkt_summary_df.to_excel(writer, sheet_name='1. Marketing', index=False)
            pivot_crm.to_excel(writer, sheet_name='2. CRM Pipeline')
            summary_ml.to_excel(writer, sheet_name='3. Sales Performance', index=False)
        st.sidebar.download_button(
            label="💾 Tải file báo cáo về máy",
            data=output.getvalue(),
            file_name="TMC_Strategic_Report.xlsx",
