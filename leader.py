...
def process_data(f_mkt, f_crm, f_ml):
    ...
    # Load Masterlife - Cố định 1625 hồ sơ
    ...
    # --- TẦNG 3: XỬ LÝ (CHỈ DÙNG MASTERLIFE) ---
    ...
    with t1:
        ...
        c11.table(mkt_summary_df)
        fig1 = px.pie(mkt_summary_df, values='Số lượng', names='Hạng mục', title="Tỷ lệ Lead MKT")
        c12.plotly_chart(fig1, use_container_width=True)
    ...
    with t3:
        ...
        c31.dataframe(summary_ml.style.format({"Tổng Doanh Thu": "${:,.0f}"}), use_container_width=True)
        c31.metric("TỔNG HỒ SƠ", f"{len(df_ml):,}")
        fig3 = px.pie(summary_ml, values='Tổng Doanh Thu', names='Nguồn', title="Cơ cấu Doanh số", hole=0.4)
        c32.plotly_chart(fig3, use_container_width=True)

    # --- NÚT EXPORT (VIẾT LẠI AN TOÀN) ---
    st.sidebar.markdown("---")
